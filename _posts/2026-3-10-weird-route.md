...
looking at the line above it we see ```if (global.flag[915] == 0 && global.flag[531] == 6)``` global.flag[915] is the main flag for determining what phase of the weird route you are currently in, if that flag is = 0 then you have not started the weird route. global.flag[531] is the encounter flag for the virovirokun that can give noelle a tutoriel on how battles work, for our purposes we just need to know that if that's = 6 it means we made noelle freeze it, simple right?\
if the weird route hasn't started yet and we just made noelle freeze the tutoriel virovirokun then play the weird route jingle!
